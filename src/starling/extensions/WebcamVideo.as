package starling.extensions
{
	...
	public class WebcamVideo extends Quad
	{
		...
		private var mVideo:Video = new Video();
		...
		private var mSmoothing:String = TextureSmoothing.NONE;
		...
		private function onCameraChange():void
		{
			if (!mTexture)
				readjustSize();
			if (mActive && !mCamera.muted && (mAddedToStage || mForcePlay))
			{
		 ...
		 *  @see stop()
		 */
		public function start(forceStart:Boolean = false):void
		{
			mActive = true;
			mRecording = true;
			mForcePlay = forceStart;
			onCameraChange();
		}
		 ...
		 * @return
		 */
		private function contextStatus():Boolean
		{
		...
		public function draw():void
		{
			...
			mStatsDrawTime.unshift(getTimer() - tTime);
		...
		public function upload():void
		{
			...
			mStatsUploadTime.unshift(getTimer() - tTime);
		 ...
		 *  Unataches the camera, removes EventListeners, disposes textures and bitmapDatas
		 */
